...
namespace MageBot.Interface
{
    public partial class IACreator : Form
    {
        ...
        public IACreator(AccountUC account)
        {
            ...
            string combinedPath = System.IO.Path.Combine(ApplicationDataPath, "MageBot", "IAs");
        ...
        private void SaveIABt_Click(object sender, EventArgs e)
        {
            if (saveFileDialog1.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                //FightConfig Config = new FightConfig(ChoixTactique.SelectedText, ChoixPlacement.SelectedText, (int)ChoixFarCells.Value); //TODO Militão 2.0: Check this
                //string ApplicationDataPath = Environment.GetFolderPath (Environment.SpecialFolder.ApplicationData);
                //string tempPath = iapath = System.IO.Path.Combine (ApplicationDataPath, "MageBot", "Temp");
                ...
                //Serialize(Disp, Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\MageBot.Temp\disp.xml");
                //Serialize(Config, Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\MageBot.Temp\config.xml");//TODO Militão 2.0: Check this
                Serialize(SpellsList, Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\MageBot.Temp\spells.xml");
                compression(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\MageBot.Temp\", iapath);
                File.Delete(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\MageBot.Temp\disp.xml");
                File.Delete(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\MageBot.Temp\config.xml");
                File.Delete(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\MageBot.Temp\spells.xml");
                this.Close();
            }
        }
        ...
        {
            if ((string)ChoixTactique.SelectedItem == "Fuyard")
            {
                sadikLabel12.Show();
                ChoixFarCells.Show();
            }
        ...
        private void compression(string DirectoryToZip, string TheZipFile)
        {
            ...
            //ZipOutputStream ZipStream = new ZipOutputStream(File.Create(TheZipFile)); //TODO Militão 2.0: Check this
            ...
            for (i = 0; i < filestozip.Length; i++)
            {
                ...
                //ZipEntry entry = new ZipEntry(filestozip[i].Replace(DirectoryToZip, "")); //TODO Militão 2.0: Check this
                // On ajoute la nouvelle entrée
                //ZipStream.PutNextEntry(entry); //TODO Militão 2.0: Check this
                // On crée la nouvelle entrée dans l'archive
                //ZipStream.Write(buffer, 0, mylength); //TODO Militão 2.0: Check this
            }
            //On ferme le flux
            //ZipStream.Finish();  //TODO Militão 2.0: Check this
            //ZipStream.Close();   //TODO Militão 2.0: Check this
        }

        private void FillSpells()
        {
            ChoixSorts.Items.Clear();
            foreach (BSpell spell in accUserControl.Account.Spells)
            {
                ChoixSorts.Items.Add(spell.Name);
            }
