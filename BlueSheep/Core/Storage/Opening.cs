    ...
    class Opening
    {
        ...
        public void Init(Account.Account account)
        {
            ...
            using (BigEndianWriter writer = new BigEndianWriter())
            {
                ...
                messagePackaging.Pack(interactiveUseRequestMessage.MessageID);

                account.SocketManager.Send(messagePackaging.Writer.Content);
                account.LastPacketID.Clear();
                account.Log(new Interface.Text.DebugTextInformation("[SND] 5001 (InteractiveUseRequestMessage)"), 0);
            }
        }
        #endregion
    }
